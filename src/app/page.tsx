...
export default async function Home() {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URI}/api/users`, {
        cache: "no-store"
    });
    const { users }: { users: RegisteredUser[] } = await res.json();
        ...
        <main className="min-h-screen bg-white dark:bg-slate-900 transition-theme">
            ...
            <nav className="sticky top-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-gray-200 dark:border-slate-700/60 px-6 py-4 transition-theme">
                <div className="max-w-6xl mx-auto flex items-center justify-between">
                    {/* Logo — same deco style as intro blocks */}
                    ...
                    <div className="flex items-center gap-2">
                        {session?.isLoggedIn ? (
                            ...
                            <>
                                <Link
                                    href="/login"
                                    className="px-4 py-2 text-sm font-medium rounded-full text-slate-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-800 transition-theme"
                                >
                                    Sign in
                                </Link>
                                <Link
                                    href="/register"
                                    className="px-4 py-2 text-sm font-medium rounded-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 hover:opacity-90 transition-opacity"
                                >
                                    Get started
                                </Link>
                            </>
            ...
            <div className="relative overflow-hidden bg-slate-900 dark:bg-slate-950 px-6 pt-20 pb-24 transition-theme">
                {/* Deco blocks — mirrors intro's intro-deco-v / intro-deco-h style */}
                ...
                <div className="relative mt-16 grid grid-cols-3 divide-x divide-slate-700 max-w-sm sm:max-w-none sm:flex sm:items-center sm:justify-center sm:gap-10 sm:divide-x-0 md:gap-20 mx-auto">
                    ...
                    {/* Divider — hidden on mobile (using divide-x instead), visible on sm+ */}
            ...
            <section className="max-w-6xl mx-auto px-6 py-16">
                ...
                {users?.length > 0 ? (
                        ...
                        {users.map((user) => (
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <h3 className="text-sm font-semibold text-slate-900 dark:text-white truncate transition-theme">
                                        {user.name}
                                    </h3>
                                    <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5 truncate transition-theme">
                                        {user.title ?? "Developer"}
                                    </p>
                                </div>

                                {/* Arrow — same style as intro's hover overlay */}
                                <div className="w-8 h-8 rounded-sm border border-gray-200 dark:border-slate-600 flex items-center justify-center flex-shrink-0 opacity-0 group-hover:opacity-100 transition-all duration-300 bg-gray-50 dark:bg-slate-700">
                                    <svg
                                        className="w-3.5 h-3.5 text-slate-500 dark:text-slate-300 group-hover:translate-x-px group-hover:-translate-y-px transition-transform duration-300"
                                        fill="none" viewBox="0 0 14 14" stroke="currentColor" strokeWidth="1.75"
                                    >
                    ...
                    <div className="flex flex-col items-center justify-center py-28 text-center">
                        ...
                        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2 transition-theme">
                            No developers yet
                        </h3>
                        ...
                        <Link
                            href="/register"
                            className="px-6 py-3 rounded-sm bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-sm font-medium hover:opacity-90 transition-opacity"
                        >
                            Get Started →
                        </Link>
                    </div>
            ...
            <footer className="border-t border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-6 py-8 transition-theme">
                <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-semibold text-slate-700 dark:text-gray-300 transition-theme">
                            DevFolio
                        </span>
                    </div>
